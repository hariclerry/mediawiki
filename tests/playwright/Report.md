...
So, this brings us to the current framework being evaluated against WebdriverIO, which is [Microsoft Playwright](https://github.com/microsoft/playwright), also known as Playwright. The evaluations progress involved the following steps;
...
[Microsoft Playwright](https://github.com/microsoft/playwright) is a Node.js library to automate Chromium, Firefox, and WebKit with a single API. Playwright is known for the following capability;
...
**Statics on  WebdriverIO and Playwright Popularity**

![Screenshot](statistics.png)

The above data was adopted from [npm trends](https://www.npmtrends.com/cypress-vs-playwright-vs-puppeteer-vs-selenium-vs-testcafe) showing some of the most popular automation test tools in terms of popularity. From the above charts, we see that WebdriverIO still trumps Playwright when it comes to popularity among the testing community.

**WebdriverIO and Playwright Mediawiki-core Tests Analysis**

![Screenshot](evaluation-chart.png)

The above visaulised chart represents data that was collected by running mediawiki/core tests in WebdriverIO and Playwright. A total of 14 tests were run using both frameworks on [Github Actions](https://docs.github.com/en/actions) platform.
...
However, it also presents some cons such as; 
...
From the various evaluations done against WebdriverIO, there seems to be great potential replacement and, Puppeteer and Microsoft Playwright seem to stand out in terms of both speed, Stability. However, the question being asked is, is it worth migrating from WebdriverIO to another test automation framework? Let’s look at the Advantages and disadvantages of a potential migration;

*Advantages*
- WebdriverIO is Javascript based and is  built over nodeJs just like Puppeteer and Microsoft Playwright thus it will be easier for developers since there would be no differences in programming language.

*Disadvantages*
- Investment in WebdriverIO tool. Most tests are already written in WebdriverIO and there are about 30 repositories being used by Mediawiki, so migrating from it to another tool would require a great deal of time and manpower.
- Update of documentation. Most documentations are already written and have references to  WebdriverIO. Updating the documentations would require some time.

### What Next
- Refactor the existing test
- Evaluate Playwright against Puppeteer
### Summary
Statistics show that Playwright is a better alternative tools over WebdriverIO in terms of speed. However, WebdriverIO seems to be a great tool given that it keeps evolving, new and modern features are being added to it and the community is still vibrant and supportive.
